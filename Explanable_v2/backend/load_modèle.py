    ...
    try:
        if 'pkl' in filename:
            ...
            try:
                ...
                log.info("quelque chose n'a pas marcher avec le chargement du modèle")
                log.error(e)

            if "predict" in dir(model):# vérification que c'est un modèle
                try:
        ...
        # TODO: Ajout de la possibilité de chargé autre modèle que pkl
        if "hdf5" in filename:
            model = keras.models.load_model(io.BytesIO(decoded)) # cgar
            if "predict" in dir(model): # vérification que c'est un modèle
                try:
                    model_name = type(model).__name__
                except Exception as e:
