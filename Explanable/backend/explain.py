...
class dalex(explain):
    ...
    def feature_importance_shap(self): # problem With this

        log.info("calcule de feature importance shap")
        try:
            return self.modelexpl.model_parts(type='shap_wrapper', shap_explainer_type="TreeExplainer").plot(show=False)
        except Exception as e:
            log.error(e)
